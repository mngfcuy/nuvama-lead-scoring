...
# ---------------------------------------------------------------
# 1. Feature engineering: structured fields only, no leakage
# ---------------------------------------------------------------
# num_existing_products: turn "PMS, AIF, Insurance" into 3, missing -> 0
...
# scale numeric features for logistic regression (RF doesn't need this,
# but scaling the shared X does no harm to RF and keeps one X for both)
...
rf.fit(X_train, y_train)  # RF uses unscaled X, tree splits don't care about scale
...
for name, (model, X_eval) in models.items():
    ...
    # pick threshold that maximizes F1 on the precision-recall curve
...
plt.tight_layout()
plt.savefig(f"{PLOTS_DIR}/05_baseline_roc_pr_curves.png", dpi=100)
plt.close()

# confusion matrices as a plot
...
plt.tight_layout()
plt.savefig(f"{PLOTS_DIR}/06_confusion_matrices.png", dpi=100)
plt.close()

print(f"\nPlots saved to {PLOTS_DIR}/")

# ---------------------------------------------------------------
# 6. Feature importance (random forest) for a quick sanity check
# ---------------------------------------------------------------
